...
const CONFIG = (
    ...
    tsel_src   = "/Volumes/MorseSSD/Pierre_Server_BACKUP/PFCmap_NHP/preprocessing/metrics_extraction/timeselections/timeselections_Carlen",
    dest       = "/Volumes/MorseSSD/CCG",

    # Epoch selection — matches __TSEL__ filename pattern
    epoch      = "prestim",
    state      = "active",
    stimtype   = "all",
    duration   = "3",
    ...
    min_fr     = 0.0005,   # minimum firing rate (spikes/bin)
...
# ── Helper: build epoch file path from filename ───────────────────────────────
function epoch_path(cfg, filename)
    joinpath(cfg.tsel_src,
        filename *
        "__TSEL" * cfg.epoch * cfg.duration *
        "__STATE" * cfg.state *
        "__" * cfg.stimtype * ".h5")
end
...
function process_recording(cfg, nwb_file)
    ...
    ep_path  = epoch_path(cfg, filename)
    ...
    if !isfile(ep_path)
        println("  ⚠ Epoch file not found — skipping")
        println("    Expected: $ep_path")
        return :skipped_no_epoch
    end
    ...
    ts_in, ts_out = load_epochs(ep_path)
    n_trials     = length(ts_in)
    epoch_dur_ms = round((ts_out[1] - ts_in[1]) * 1000, digits=1)
    win_bins     = round(Int, (ts_out[1] - ts_in[1]) / cfg.binsize)
    println("    $n_trials epochs  |  $epoch_dur_ms ms  |  $win_bins bins/trial")
    ...
    unit_i_sig    = [kept_ids[pairs[k][1]] for k in sig_idx]
    unit_m_sig    = [kept_ids[pairs[k][2]] for k in sig_idx]
...
# ── Run loop ──────────────────────────────────────────────────────────────────
t_batch_start = time()
stats = Dict(:success => 0, :skipped_no_epoch => 0,
             :skipped_no_neurons => 0, :error => 0)

for (f_idx, nwb_file) in enumerate(nwb_files)
    println("\n[$f_idx / $(length(nwb_files))]")
...
@printf "  Successful     : %d\n"  stats[:success]
@printf "  No epoch file  : %d\n"  stats[:skipped_no_epoch]
